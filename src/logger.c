...
void logger_config( const char *df, const char *ll ) {
	...
	logger_set_date_format(df);
	logger_set_log_level(ll);
}
...
void logger_set_date_format( const char *df ) {
    if (df != NULL) {
        strcpy(dateformat, df);
    }
}

void logger_set_log_level( const char *ll ) {
    if (ll != NULL) {
        for (level = 0; level < 5; level++) {
            if (strcmp(log_names[level], ll) == 0) {
                log_level = level;
                break;
            }
        }
