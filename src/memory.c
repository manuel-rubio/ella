...
void ews_memory_get_units( char *buffer, long units ) {
...
void ews_memory_stats( void ) {
    ...
    ews_memory_get_units(buffer + strlen(buffer), ews_memory_allocated);
    ews_verbose(LOG_LEVEL_INFO, buffer);

    strcpy(buffer, "Freed memory: ");
    ews_memory_get_units(buffer + strlen(buffer), ews_memory_freed);
    ews_verbose(LOG_LEVEL_INFO, buffer);

    strcpy(buffer, "Lost memory: ");
    ews_memory_get_units(buffer + strlen(buffer), ews_memory);
    ews_verbose(LOG_LEVEL_INFO, buffer);

    ews_verbose(LOG_LEVEL_INFO, "Max. simultaneous allocations: %d", ews_max_simult_allocs);

    strcpy(buffer, "Max. memory in use: ");
    ews_memory_get_units(buffer + strlen(buffer), ews_max_memory_in_use);
    ews_verbose(LOG_LEVEL_INFO, buffer);
